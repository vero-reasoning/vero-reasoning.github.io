...
sources = ["ground1.html", "ground2.html", "ground3.html"]
items = []
...
for i, name in enumerate(sources, start=1):
    ...
    bbox_img_match = re.search(r'class="assistant-scroll"[\s\S]*?</(?:pre|div)>[\s\S]*?<img[^>]+src="([^"]+)"[^>]*>', text)
    ...
    bbox_image = bbox_img_match.group(1).strip() if bbox_img_match else ""
...
<body>
            ...
            <div class="chat-body">
                ...
                <article class="assistant-turn" aria-label="Model response">
                    ...
                    <div class="assistant-content">
                        ...
                        <img id="bboxImage" src="" alt="Bounding box visualization"
                            style="width:100%;border:1px solid #dde1ec;border-radius:8px;display:none;margin-top:12px;" />
                    </div>
                </article>
            </div>
        </section>
    ...
    <script>
        ...
        function render() {{
            ...
            if (item.bboxImage) {{
                bboxImage.src = item.bboxImage;
                bboxImage.style.display = 'block';
            }} else {{
                bboxImage.src = '';
...
(base / "ground123.html").write_text(html, encoding="utf-8")
print("Wrote static ground123.html with inline DEMOS from ground1/2/3")
